        ...
        <div class="px-[25px] w-full h-[329px] mb-[61px]">
          <section class="flex flex-col gap-8">
            <div class="flex flex-col gap-[15px]">
              <h3 class="uppercase font-manrope font-[700] text-[#7A7A7A] text-[12px] tracking-widest">Recette</h3>
              <p class="font-manrope text-[#1B1B1B] text-[14px] text-[400] line-clamp-3">${
                recipe.description
              }</p>
            </div>
            <div class="flex flex-col gap-[15px]">
              <h3 class="uppercase font-manrope font-[700] text-[#7A7A7A] text-[12px] tracking-widest">Ingrédients</h3>
              <ul class="grid grid-cols-2 gap-y-[21px]">
                ${recipe?.ingredients.map(generateIngredientListItem).join("")}
              </ul>
            </div>
          </section>
        </div>
...
export function displayRecipes(recipes) {
  ...
  const cards = recipes?.map(generateRecipeCard).join("");
